...
async function run() {
    ...
    try {
        ...
        // === 13. Create Security Report Issue (on push, not PR) ===
        if (githubToken && !github.context.payload.pull_request) {
            await createSecurityReportIssue(githubToken, filteredVulns, score, duration);
        }
...
async function createSecurityReportIssue(token, vulns, score, duration) {
    const octokit = github.getOctokit(token);
    const context = github.context;
    const issueTitle = 'SynapseAudit Security Report';
    ...
    try {
        ...
        core.warning(`Failed to check existing issues: ${e.message}`);
    }
    ...
    let body = `## 🛡️ SynapseAudit Security Scan Report\n\n`;
    ...
    body += `| **Total** | **${vulns.length}** |\n\n`;
    
    if (vulns.length === 0) {
        body += `✅ **No vulnerabilities detected!** Your codebase is secure.\n\n`;
    } else {
        body += `### Top Findings\n\n`;
        body += `| Severity | Type | File | Line |\n|---|---|---|---|\n`;
        ...
        if (vulns.length > 20) {
            body += `\n*...and ${vulns.length - 20} more findings.*\n`;
        }
        
        body += `\n### Recommended Actions\n`;
        body += `1. Review the findings above\n`;
        body += `2. Address Critical and High severity issues first\n`;
        body += `3. Run \`npx synapse-audit fix\` for automatic remediation\n`;
    }
    
    body += `\n---\n_Auto-generated by [SynapseAudit](https://synapseaudit.digidenone.tech) on ${new Date().toLocaleDateString()}_`;
    ...
    try {
        if (existingIssue) {
            ...
            core.info(`Updated security report issue #${existingIssue.number}`);
        } else {
            // Create new issue
            ...
            core.info(`Created security report issue #${newIssue.number}`);
        }
    } catch (error) {
